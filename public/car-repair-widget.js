...
(function() {
    ...
    const styles = `/* Existing styles unchanged */`;

    // HTML structure (updated accept attribute for clarity)
    const widgetHTML = `
        <div id="car-repair-widget">
            <button class="crw-toggle-btn" id="crw-toggle" title="Get Car Repair Estimate">
                🚗
                ...
                <div class="crw-modal-content">
                    ...
                    <div class="crw-body">
                        <form id="crw-form" enctype="multipart/form-data">
                            ...
                            <div class="crw-form-group">
                                <label class="crw-label" for="image">Upload Damage Image</label>
                                <input type="file" class="crw-file" id="image" name="image" accept="image/jpeg,image/png,image/heic,image/heif" required>
                            </div>
                            <div class="crw-checkbox-group">
                                <input type="checkbox" id="luxuryVehicle" name="isLuxury" class="crw-checkbox">
                                <label for="luxuryVehicle" class="crw-label">Luxury Vehicle</label>
    ...
    async function convertHeicToJpeg(file) {
        ...
        // Dynamically load heic2any library if needed
        ...
        try {
            ...
            throw new Error('Failed to convert HEIC image to JPEG. Please upload a JPEG or PNG image.');
        }
    }
    ...
    function initWidget() {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            if (file.size > 10 * 1024 * 1024) {
                displayError('Image size exceeds 10MB. Please upload a smaller file.');
                e.target.value = '';
                return;
            }
            ...
            if (!validTypes.includes(file.type)) {
                displayError('Please upload a JPEG, PNG, HEIC, or HEIF image.');
                e.target.value = '';
                return;
            }
        });
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (originalFile && (originalFile.type.includes('heic') || originalFile.type.includes('heif'))) {
                    fileToUpload = await convertHeicToJpeg(originalFile);
                ...
                const errorMessage = error.message === 'The uploaded image does not contain a car' 
                    ? 'Please upload an image of a car.'
                    : error.message === 'Only JPEG, PNG, HEIC, or HEIF images are allowed' 
                    ? 'Please upload a JPEG, PNG, HEIC, or HEIF image.'
                    : error.message === 'Image file is required' 
                    ? 'Please upload an image.'
                    : error.message === 'Car make and year are required' 
                    ? 'Please select both car make and year.'
                    : error.message.includes('Failed to convert HEIC') 
                    ? error.message
                    : 'An error occurred while fetching the estimate. Please try again.';
                displayError(errorMessage);
            } finally {
        ...
        // Remove iOS-specific HEIC warning since we handle conversion
    }
