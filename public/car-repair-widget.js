...
(function() {
    ...
    // HTML structure (updated to include HEIC/HEIF in accept attribute)
    const widgetHTML = `
        <div id="car-repair-widget">
            <button class="crw-toggle-btn" id="crw-toggle" title="Get Car Repair Estimate">
                🚗
    ...
    function initWidget() {
        ...
        // Log file selection details for debugging
        ...
        // iOS-specific warning for HEIC images
        if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && !/image\/heic/.test(fileInput.accept)) {
            fileInput.addEventListener('change', () => {
                const file = fileInput.files[0];
                if (file && file.type === 'image/heic') {
                    displayError('For best results, convert HEIC images to JPEG or PNG before uploading.');
                }
            });
        }
    }
